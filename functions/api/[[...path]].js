/**
 * EdgeOne Pages Functions 动态路由处理器 - 测试版本
 * 尝试不同的文件命名语法：[[...path]].js
 */
...
export async function onRequest(context) {
    ...
    // 详细的路径解析调试
    console.log('=== EdgeOne Functions Debug (测试版本) ===');
    ...
    try {
        ...
        if (segments.length === 1 && segments[0] === 'debug') {
            console.log('Processing /api/debug endpoint (测试版本)');
            return setCorsHeaders(new Response(JSON.stringify({
                success: true,
                message: '调试端点工作正常！(测试版本)',
                timestamp: new Date().toISOString(),
                version: '测试版本 - [[...path]].js',
                debug: {
                    url: url.href,
                    pathname: url.pathname,
                    method: method,
        ...
        if (segments.length === 0) {
            console.log('Processing /api/ root endpoint (测试版本)');
            return setCorsHeaders(new Response(JSON.stringify({
                message: 'EdgeOne Pages Functions API (测试版本)',
                version: '测试版本 - [[...path]].js',
                timestamp: new Date().toISOString(),
                availableEndpoints: [
                    '/api/test',
        ...
        if (segments.length === 1 && segments[0] === 'test') {
            console.log('Processing /api/test endpoint (测试版本)');
            return setCorsHeaders(new Response(JSON.stringify({
                success: true,
                message: 'EdgeOne Functions API 工作正常！(测试版本)',
                timestamp: new Date().toISOString(),
                version: '测试版本 - [[...path]].js'
            }), {
                status: 200,
        ...
        if (segments.length === 1 && segments[0] === 'configs') {
            console.log('Processing /api/configs endpoint (测试版本)');
            return setCorsHeaders(new Response(JSON.stringify({
                success: true,
                message: 'Configs endpoint working (测试版本)',
                timestamp: new Date().toISOString(),
        ...
        if (segments.length === 2 && segments[0] === 'config') {
            ...
            console.log('Processing /api/config/' + configKey + ' endpoint (测试版本)');
            
            // 简化测试：直接返回成功响应
            return setCorsHeaders(new Response(JSON.stringify({
                success: true,
                configKey: configKey,
                data: { test: 'data', message: '测试数据' },
                source: '测试版本',
                timestamp: new Date().toISOString()
            }), {
                status: 200,
